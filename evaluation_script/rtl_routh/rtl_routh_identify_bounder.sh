...
while (( $(echo "$p_to_check <= $p_end" | bc -l) )); do
    # find largest i that makes sys unstable
    i_start=0.02
    i_end=1
    i_step=0.01
    i_adjust_step=0.01
    ...
    while (( $(echo "$i_to_check >= $i_start" | bc -l) )); do
        ...
        echo "::Execute Fuzz"
        CONTAINER_ID=$(sudo docker run --cpuset-cpus ${DOCKER_CPUSET} -d --rm -v $ARDUPILOT_FUZZ_HOME/routh_data_dir/${ESTIMATE_ABNORMAL_CONFIG}/..:/ardu-sim/pids -v $ARDUPILOT_FUZZ_HOME/routh_data_dir/${ESTIMATE_ABNORMAL_CONFIG}/logs:/ardu-sim/logs rtl_mode:latest)
        sudo docker wait $CONTAINER_ID
        PID_PARAMETERS="PSC_VELXY_P,PSC_VELXY_I,PSC_VELXY_D"
        CONTAINER_ID=$(docker run --cpuset-cpus ${DOCKER_CPUSET}  -d --rm -e PID_PARMS=${PID_PARAMETERS} -v $ARDUPILOT_FUZZ_HOME/routh_data_dir/${ESTIMATE_ABNORMAL_CONFIG}/logs:/ardu-sim/logs  -v $ARDUPILOT_FUZZ_HOME/routh_data_dir/${ESTIMATE_ABNORMAL_CONFIG}/res:/ardu-sim/results rtl_oracle:latest)
        sudo docker wait $CONTAINER_ID
    ...
    while (( $(echo "$d_to_check <= $d_end" | bc -l) )); do
        ...
        # suppose we already have the rtl_oracle:latest as the oracle
        PID_PARAMETERS="PSC_VELXY_P,PSC_VELXY_I,PSC_VELXY_D"
        CONTAINER_ID=$(sudo docker run --cpuset-cpus ${DOCKER_CPUSET}  -d --rm -e PID_PARMS=${PID_PARAMETERS} -v $ARDUPILOT_FUZZ_HOME/routh_data_dir/${ESTIMATE_ABNORMAL_CONFIG}/logs:/ardu-sim/logs  -v $ARDUPILOT_FUZZ_HOME/routh_data_dir/${ESTIMATE_ABNORMAL_CONFIG}/res:/ardu-sim/results rtl_oracle:latest)
        sudo docker wait $CONTAINER_ID

        # We use awk the extract the last field of the result file
        outputfile=`ls $ARDUPILOT_FUZZ_HOME/routh_data_dir/${ESTIMATE_ABNORMAL_CONFIG}/res/*.result`
        output=$(awk '{print $NF}' "$outputfile")
            output=$(echo "$output" | bc -l)

        # clean up the logs and the result file
        rm -rf $ARDUPILOT_FUZZ_HOME/routh_data_dir/${ESTIMATE_ABNORMAL_CONFIG}/logs/*.BIN
        rm -rf $ARDUPILOT_FUZZ_HOME/routh_data_dir/${ESTIMATE_ABNORMAL_CONFIG}/logs/LASTLOG.TXT
        rm -rf $ARDUPILOT_FUZZ_HOME/routh_data_dir/${ESTIMATE_ABNORMAL_CONFIG}/res/*.result
