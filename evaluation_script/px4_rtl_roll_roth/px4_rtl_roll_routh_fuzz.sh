...
while (( $(echo "$p_to_check <= $p_end" | bc -l) )); do
    ...
    while (( $(echo "$i_to_check >= $i_start" | bc -l) )); do
        ...
        CONTAINER_ID=$(docker run --cpus=6  -d --rm -e PID_PARMS=${PID_PARAMETERS} -v $PX4_FUZZ_HOME/routh_fuzz_data_dir/${ESTIMATE_ABNORMAL_CONFIG}/logs:/px4/logs  -v $PX4_FUZZ_HOME/routh_fuzz_data_dir/${ESTIMATE_ABNORMAL_CONFIG}/res:/px4/results px4_rtl_oracle)
        sudo docker wait $CONTAINER_ID
