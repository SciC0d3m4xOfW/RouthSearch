...
while (( $(echo "$p_to_check <= $p_end" | bc -l) )); do
    ...
    while (( $(echo "$i_to_check >= $i_start" | bc -l) )); do
        ...
        echo "::Execute Fuzz"
        CONTAINER_ID=$(sudo docker run --cpuset-cpus ${DOCKER_CPUSET} -d --rm -v $ARDUPILOT_FUZZ_HOME/routh_data_dir/${ESTIMATE_ABNORMAL_CONFIG}/..:/ardu-sim/pids -v $ARDUPILOT_FUZZ_HOME/routh_data_dir/${ESTIMATE_ABNORMAL_CONFIG}/logs:/ardu-sim/logs brake_mode:latest)
        sudo docker wait $CONTAINER_ID
        PID_PARAMETERS="PSC_VELXY_P,PSC_VELXY_I,PSC_VELXY_D"
        CONTAINER_ID=$(docker run --cpuset-cpus ${DOCKER_CPUSET}  -d --rm -e PID_PARMS=${PID_PARAMETERS} -v $ARDUPILOT_FUZZ_HOME/routh_data_dir/${ESTIMATE_ABNORMAL_CONFIG}/logs:/ardu-sim/logs  -v $ARDUPILOT_FUZZ_HOME/routh_data_dir/${ESTIMATE_ABNORMAL_CONFIG}/res:/ardu-sim/results brake_oracle:latest)
        sudo docker wait $CONTAINER_ID
    ...
    while (( $(echo "$d_to_check <= $d_end" | bc -l) )); do
        ...
        PID_PARAMETERS="PSC_VELXY_P,PSC_VELXY_I,PSC_VELXY_D"
        CONTAINER_ID=$(sudo docker run --cpuset-cpus ${DOCKER_CPUSET}  -d --rm -e PID_PARMS=${PID_PARAMETERS} -v $ARDUPILOT_FUZZ_HOME/routh_data_dir/${ESTIMATE_ABNORMAL_CONFIG}/logs:/ardu-sim/logs  -v $ARDUPILOT_FUZZ_HOME/routh_data_dir/${ESTIMATE_ABNORMAL_CONFIG}/res:/ardu-sim/results brake_oracle:latest)
        sudo docker wait $CONTAINER_ID
